...
@section('content')
<div class="container">
    <h1>Edit Patient Medical Record</h1>
    <div class="my-3">
        <a href="{{ route('medical_records.index') }}" class="btn btn-primary"><i class="fas fa-arrow-left"></i> Back</a>
    </div>
    <div class="row">
                            ...
                            <tr>
                                ...
                                <td>
                                    <div class="btn-group" role="group">
                                        <a href="#" data-toggle="modal" data-target="#editRecordModal{{ $record->id }}">
                                            <i class="fas fa-edit text-secondary mr-1" title="Edit"></i>
                                        </a>
                                        <a href="#" data-toggle="modal" data-target="#deleteRecordModal{{ $record->id }}">
            ...
            <div class="modal-dialog modal-lg" role="document">
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        <form method="POST" action="{{ route('records.update', ['id' => $record->id]) }}">
                            @csrf
                            @method('PATCH')
                                ...
                                <div class="col-md-6">
                                    ...
                                    <label for="type" class="form-label">Appointment Type</label>
                                        <select class="form-select" id="type" name="type">
                                            <option value="consultation" {{ $record->appointment->type === 'consultation' ? 'selected' : '' }}>General Consultation</option>
                                            <option value="checkup" {{ $record->appointment->type === 'checkup' ? 'selected' : '' }}>Checkup</option>
                                            <option value="follow-up" {{ $record->appointment->type === 'follow-up' ? 'selected' : '' }}>Follow-up Visit</option>
                                            <option value="vaccination" {{ $record->appointment->type === 'vaccination' ? 'selected' : '' }}>Tetanus Vaccination</option>
                                            <option value="urgent" {{ $record->appointment->type === 'urgent' ? 'selected' : '' }}>Urgent Care</option>
                                            <option value="medcert" {{ $record->appointment->type === 'medcert' ? 'selected' : '' }}>Medical Certification</option>
                                        </select>
                                    </div>
                                    <div class="form-group">
            ...
            <div class="modal-dialog" role="document">
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        ...
                        <form method="POST" action="{{ route('records.destroy', ['id' => $record->id]) }}">
                            @csrf
                            @method('DELETE')
